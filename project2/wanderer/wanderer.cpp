...
void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
	...
	for( int i = 0; i < msg->ranges.size(); i++ )
	{
		...
		/*********************************/
			...
			if (sqrt(pow(xdist, 2.0) + pow(ydist, 2.0)) < front_dist){
				front_dist = sqrt(pow(xdist, 2.0) + pow(ydist, 2.0));
			}
			...
			if (sqrt(pow(xdist, 2.0) + pow(ydist, 2.0)) < right_dist){
				right_dist = sqrt(pow(xdist, 2.0) + pow(ydist, 2.0));
			}
			...
			// closer than the closest left point so far?
				...
				// the current distance
			if (sqrt(pow(xdist, 2.0) + pow(ydist, 2.0)) < left_dist){
				left_dist = sqrt(pow(xdist, 2.0) + pow(ydist, 2.0));
			}
